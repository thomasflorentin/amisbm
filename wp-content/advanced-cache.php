...
// WP-Optimize advanced-cache.php (written by version: 3.7.1) (homeurl: http://amisbm.localhost/) (abspath: /Users/thomas/Sites/_projets/amisbm/htdocs/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'/Users/thomas/Sites/_projets/amisbm/htdocs/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://amisbm.localhost/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/Users/thomas/Sites/_projets/amisbm/htdocs/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-amisbm.localhost.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-amisbm.localhost.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-amisbm.localhost.php');
}
